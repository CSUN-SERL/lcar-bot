...
void TraverseDirectory(string);
Mat ProcessImage(string, string);
void ExtractFeatures(Mat, string);
...
int ratio = 3;
int kernel_size = 3;
string IMAGES_DIR = "/home/n8/Pictures/images";
...
void TraverseDirectory(string path) {
    ...
    while ((dirEntry = readdir(tree)) != NULL) {
        ...
        if (dirEntry->d_type == DT_DIR && dirEntry->d_name[0] != '.') {
            ...
            Mat greyImgMat = ProcessImage(path, dirEntry->d_name);
            ExtractFeatures(greyImgMat, dirEntry->d_name);
        }
    }
